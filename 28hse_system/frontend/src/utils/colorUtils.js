 ...
 */
export function getRentRatioColor(ratio, min, max) {
  // 无租售比数据时显示灰色
  ...
  if (ratio < PIVOT) {
    ...
    // 假设4%为起点，10%以上为满红
    const normalized = Math.min((ratio - PIVOT) / 6, 1)  // (4%, 10%] -> [0, 1]
 ...
 * @returns {Array} 颜色数组
 */
export function getGradientColors(steps = 10) {
  const colors = []
  // 生成从0%到10%的渐变
  for (let i = 0; i < steps; i++) {
    const ratio = (i / (steps - 1)) * 10  // 0% -> 10%
    // 使用相同的颜色映射逻辑
    let hue
    if (ratio < 4) {
      ...
      const normalized = Math.min((ratio - 4) / 6, 1)
