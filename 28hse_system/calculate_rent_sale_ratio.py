...
def cluster_by_room_type(transactions: List[Dict]) -> Dict[int, List[Dict]]:
    """
...
def calculate_rent_sale_ratio(base_dir: str = "./28hse/transaction_records_trans",
                               enable_outlier_removal: bool = True) -> Dict:
    """
    计算平均租售比
    ...
    for estate_id in sorted(common_ids, key=lambda x: int(x) if x.isdigit() else 0):
        ...
        # 对买卖和出租数据分别进行户型聚类
        buy_clusters = cluster_by_room_type(buy_transactions)
        rent_clusters = cluster_by_room_type(rent_transactions)
        
        logger.info(f"  买卖数据户型聚类: {len(buy_clusters)} 个户型")
        logger.info(f"  出租数据户型聚类: {len(rent_clusters)} 个户型")
        
        # 计算整体租售比
        overall_sale_prices = extract_valid_values(buy_transactions, 'price_per_sqft')
        overall_rent_prices = extract_valid_values(rent_transactions, 'rent_per_sqft')
        ...
        for buy_area in sorted(buy_clusters.keys()):
            ...
            # 提取价格数据
            cluster_sale_prices = extract_valid_values(buy_cluster_trans, 'price_per_sqft')
            cluster_rent_prices = extract_valid_values(rent_cluster_trans, 'rent_per_sqft')
...
def main():
    ...
    # 计算租售比
    results = calculate_rent_sale_ratio(base_dir="./28hse/transaction_records_20260215_trans")
